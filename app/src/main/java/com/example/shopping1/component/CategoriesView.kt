                ...
                .get().addOnCompleteListener() {
                    if(it.isSuccessful){
                        val resultList = it .result.documents.mapNotNull {
                            it.toObject(CategoriesModel::class.java)
                        }
...
fun CategoryItem(category : CategoriesModel) {
    Card (modifier = Modifier.size(100.dp).clickable{ // clickable
        GlobalNavigation.navController.navigate("categoryPage/" + category.id)
    },
